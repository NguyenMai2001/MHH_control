...
figure(1)
 ...
 figure(3)
    ...
    legend('eX: sai so truc X','eY: sai so truc Y','eZ: sai so truc Z')
    hold on
    grid on
...
for t_q3=0:0.01:1
    x_khau_3=sin(q1+q2)*t_q3*q3+L2*cos(q1+q2)+L1*cos(q1);
    y_khau_3=-cos(q1+q2)*t_q3*q3+L2*sin(q1+q2)+L1*sin(q1);
    z_khau_3=0;
    plot3(x_khau_3,y_khau_3,z_khau_3,'g.',sin(q1+q2)*q3+L2*cos(q1+q2)+L1*cos(q1),-cos(q1+q2)*q3+L2*sin(q1+q2)+L1*sin(q1),0,'r.')
    hold on
    grid on
end
