...
void __interrupt() isr1()
{
    ...
    if (T0IF == 1) // phat hien co bao ngat
    {
        ...
        if (OpenServo == 1)
        {
            ...
            if (cnt_time_delay >= time_delay) // khi timer dem du time
            {
                OpenServo = 0;
            }
        }
...
void EEPROM_Write(uint8_t Address, uint8_t Data)
{
    ...
    EEADR = Address; //giu dia chi cua vi tri EEPROM dang duoc truy cap
    EEDATA = Data; //giu du lieu 8 bit de doc/ghi
    EEPGD = 0; //xac dinh quyen truy cap bo nho, khi clear -> hd tren bo nho du lieu, khi set hd tren bo nho chuong trinh
    EECON1bits.WREN = 1; //cho phep ghi hoac xoa hd \\EEPROM Write Enable bit
    GIE = 0;
    EECON2 = 0x55;
    EECON2 = 0xAA;
    EECON1bits.WR = 1;// Write Control bit, bit se duoc xoa boi phan cung khi hoan tat
    GIE = 1;
    EECON1bits.WREN = 0; // chu ky ghi vao eeprom da hoan tat
    EECON1bits.WR = 0;
}
